...
namespace FanPage.Infrastructure.Implementations
{
    public class AccountServiсe : IAccount
    {
        ...
        public AccountServiсe(CustomizationSettingsService customizationSettingsService,IPasswordManager passwordManager, SignInManager<User> signInManager, IJwtTokenManager jwtTokenManager,
        ...
        public async Task ChangePassword(ChangePasswordDto changePassword, HttpRequest request)
        {
            ...
            var checkPasswordResult = await _signInManager.CheckPasswordSignInAsync(user, changePassword.Password, false);
        ...
        public async Task RestorePassword(RestorePasswordDto restore)
        {
            ...
            {
                Subject = "FanPage",
                Html = $"Link Reset Password - {newPassword}",
                EmailTo = user.Email
            };
        ...
        public async Task ConfirmEmail(ConfirmEmailDto confirmEmail)
        {
            ...
            {
                throw new AggregateException(confirmResult.Errors.Select(s => new ResetPasswordException(s.Description)));
            }
        }
        ...
        public async Task Registration(RegistrationDto registration)
        {
            try
            {
                ...
                if (await _userManager.FindByEmailAsync(registration.Email) == null)
                {
                    ...
                    {
                        throw new AggregateException(createResult.Errors.Select(s => new UserCreateException(s.Description)));
                    }
            ...
            {
                throw new UserCreateException(e.Message);
            }
        }
