...
namespace FanPage.Domain.Fanfic.Repos.Impl
{
    public class CategoryRepository : RepositoryBase<Category>, ICategoryRepository
    {
        ...
        public CategoryRepository(FanficContext context, IMapper mapper)
            : base(context)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<CategoryDto> GetByNameAsync(string name)
        {
            var category = await _context
                .Categories.Where(x => x.Name == name)
                .FirstOrDefaultAsync();
        ...
        {
            var existingEntry = await _context.FanficCategories.FirstOrDefaultAsync(x =>
                x.FanficId == fanficId && x.CategoryId == categoryId
            );

            if (existingEntry == null)
            {
                var fanficCategory = new FanficCategory
                {
                    FanficId = fanficId,
        ...
        {
            var fanficCategory = await _context
                .FanficCategories.Where(x => x.FanficId == fanficId && x.CategoryId == categoryId)
                .FirstOrDefaultAsync();
        ...
        {
            var category = await _context
                .FanficCategories.Where(x => x.FanficId == fanficId)
                .Select(s => s.Category)
                .ToListAsync();
